...
import { NavInfo, Pdu1Person, PDU1 } from 'declarations/pd'
...
export const validatePDU1Edit = (v: Validation, t: TFunction, {
  ...
  const statsborgerskaper: Array<string> = _.get(PDU1, `${personID}.statsborgerskap`)
  hasErrors.push(validateStatsborgerskaper(v, t, { statsborgerskaper, namespace: `personmanager-${personID}-statsborgerskap` }))

  const adresse: Adresse = _.get(PDU1, `${personID}.adresse`)
  ...
  const sisteAnsettelseInfo: GrunnTilOpphør = _.get(PDU1, 'opphoer')
