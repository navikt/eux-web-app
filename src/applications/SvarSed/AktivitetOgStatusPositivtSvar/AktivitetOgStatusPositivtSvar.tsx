...
import { Aktivitet, AktivitetType, PersonensStatus, PositivtSvar, H006Sed } from 'declarations/h006'
...
import {
  ...
  validateAktivitetOgStatusPositivtSvar,
...
const AktivitetOgStatusPositivtSvar: React.FC<MainFormProps> = ({
  label,
  parentNamespace,
  personID,
  personName,
  ...
  const namespace = `${parentNamespace}-${personID}-positivtsvar-status`
  const target = 'positivtSvar.aktiviteter'
  const sed = replySed as H006Sed
  const positivtSvar: PositivtSvar | undefined = sed.positivtSvar
  const aktiviteter: Array<Aktivitet> | undefined = positivtSvar?.aktiviteter
  ...
  useUnmount(() => {
    ...
    performValidation<ValidationAktivitetOgStatusPositivtSvarProps>(
      clonedvalidation, namespace, validateAktivitetOgStatusPositivtSvar, {
        replySed: sed,
        personName
      }, true
    )
  ...
  const setPersonensStatus = (value: string) => {
    ...
    dispatch(updateReplySed('positivtSvar.personensStatus', newStatus))
    if (newStatus !== 'annet') {
      dispatch(updateReplySed('positivtSvar.personensStatusAnnet', undefined))
    }
  ...
  const setPersonensStatusAnnet = (value: string) => {
    dispatch(updateReplySed('positivtSvar.personensStatusAnnet', value.trim() || undefined))
  ...
  const setInfoField = (id: string, value: string) => {
    dispatch(updateReplySed(`positivtSvar.${id}`, value.trim() || undefined))
    ...
    <Box padding="space-16" borderWidth="1" borderColor="neutral-subtle" borderRadius="8">
      <VStack gap="space-16">
        ...
        <Select
          ...
          value={positivtSvar?.personensStatus ?? ''}
        ...
        {positivtSvar?.personensStatus === 'annet' && (
          <TextArea
            ...
            value={positivtSvar?.personensStatusAnnet}
            required
          />
        )}
        ...
        <TextArea
          ...
          value={positivtSvar?.inntektskildeStudenter}
        />
        <TextArea
          ...
          value={positivtSvar?.bosituasjonHvorPermanent}
        />
        <HGrid columns={{ xs: 1, md: 2 }} gap="space-16" align="start">
          <Input
            ...
            value={positivtSvar?.permanentOppholdSkattemessig}
          />
        </HGrid>
      </VStack>
    </Box>
...
export default AktivitetOgStatusPositivtSvar
